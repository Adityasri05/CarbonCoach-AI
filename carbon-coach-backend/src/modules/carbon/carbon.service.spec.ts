...
describe("CarbonService", () => {
  ...
  describe("logActivity", () => {
    it("should log activity and award leaderboard points", async () => {
      ...
      const mockLog = { id: "log-1", userId, activityType, quantity, unit, emissionsCalculated: 1.8 };
      const mockLeaderboard = { userId, totalPoints: 100 };

      jest.spyOn(prisma.activityLog, "create").mockResolvedValue(mockLog as any);
      jest.spyOn(prisma.leaderboard, "findUnique").mockResolvedValue(mockLeaderboard as any);
      jest.spyOn(prisma.leaderboard, "update").mockResolvedValue(mockLeaderboard as any);

      const result = await service.logActivity(userId, activityType, quantity, unit);
  ...
  describe("getHistory", () => {
    it("should retrieve recent activities", async () => {
      ...
      jest.spyOn(prisma.activityLog, "findMany").mockResolvedValue(mockHistory as any);
  ...
  describe("getSummary", () => {
    it("should return the formatted summary breakdown", async () => {
      ...
      jest.spyOn(prisma.carbonRecord, "groupBy").mockResolvedValue([
        { category: ActivityCategory.TRANSPORTATION, _sum: { emissions: 12.5 } },
