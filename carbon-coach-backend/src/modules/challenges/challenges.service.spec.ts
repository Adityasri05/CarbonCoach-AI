...
describe("ChallengesService", () => {
  ...
  describe("getChallenges", () => {
    it("should return joined and unjoined challenges correctly mapped", async () => {
      ...
      const mockParticipations = [
        { challengeId: "c1", status: ChallengeStatus.JOINED, daysCompleted: 2, progress: 50 },
      ];

      jest.spyOn(prisma.challenge, "findMany").mockResolvedValue(mockChallenges as any);
      jest.spyOn(prisma.challengeParticipation, "findMany").mockResolvedValue(mockParticipations as any);

      const result = await service.getChallenges("user-1");

      expect(result).toEqual([
        { id: "c1", title: "Challenge 1", active: true, status: "JOINED", daysCompleted: 2, progress: 50 },
        { id: "c2", title: "Challenge 2", active: true, status: "NOT_JOINED", daysCompleted: 0, progress: 0 },
      ]);
    });
  });
  ...
  describe("joinChallenge", () => {
    it("should allow a user to join an active challenge", async () => {
      ...
      jest.spyOn(prisma.challenge, "findUnique").mockResolvedValue(mockChallenge as any);
      jest.spyOn(prisma.challengeParticipation, "findUnique").mockResolvedValue(null);
      jest.spyOn(prisma.challengeParticipation, "create").mockResolvedValue({ id: "p1" } as any);
    ...
    it("should throw NotFoundException if challenge does not exist", async () => {
      ...
      await expect(service.joinChallenge("user-1", "c-none")).rejects.toThrow(NotFoundException);
    });

    it("should throw BadRequestException if user already joined", async () => {
      jest.spyOn(prisma.challenge, "findUnique").mockResolvedValue({ id: "c1" } as any);
      jest.spyOn(prisma.challengeParticipation, "findUnique").mockResolvedValue({ id: "p1" } as any);

      await expect(service.joinChallenge("user-1", "c1")).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("logProgress", () => {
    it("should increment days completed and update progress percentage", async () => {
      const mockPart = {
        ...
        status: ChallengeStatus.JOINED,
        daysCompleted: 1,
        challenge: { id: "c1", daysTotal: 5, points: 100, title: "Title", description: "Desc" },
      };

      jest.spyOn(prisma.challengeParticipation, "findUnique").mockResolvedValue(mockPart as any);
      jest.spyOn(prisma.challengeParticipation, "update").mockResolvedValue({ id: "p1", progress: 40 } as any);
    ...
    it("should complete challenge and award points, notification, badge when total days hit", async () => {
      ...
      jest.spyOn(prisma.challengeParticipation, "findUnique").mockResolvedValue(mockPart as any);
      jest.spyOn(prisma.challengeParticipation, "update").mockResolvedValue({ id: "p1", status: ChallengeStatus.COMPLETED } as any);
    ...
    it("should throw NotFoundException if participation is missing", async () => {
      ...
      await expect(service.logProgress("user-1", "c1", 1)).rejects.toThrow(NotFoundException);
    });
    ...
    it("should throw BadRequestException if already completed", async () => {
      ...
      await expect(service.logProgress("user-1", "c1", 1)).rejects.toThrow(BadRequestException);
    });
  });
});
