...
- Recycling Habits: ${habits.recyclingHabits || "Unknown"}` : "User Profile Info: No profile habits created yet."}`;
    ...
    try {
      ...
      botReply = response.text || "I'm sorry, I couldn't generate a response. Please try again.";
    } catch (error) {
      console.error("Gemini API Error, falling back to rule-based response:", error);
      // Fallback RAG / rule-based response
      botReply = "That's an insightful question. Reducing emissions starts with understanding utility grids and transport efficiencies. Would you like to check your AI Carbon Twin simulation?";
      const lower = userMessage.toLowerCase();
      if (lower.includes("how can i reduce") || lower.includes("ways to reduce")) {
        botReply = "To lower your carbon footprint: \n1. Optimize your AC (set temperature to 25°C).\n2. Reduce red meat meals (beef has a high carbon lifecycle cost).\n3. Transition to public transport, hybrid, or EV travel.";
      } else if (lower.includes("cycling") || lower.includes("bike")) {
        botReply = "Cycling generates 0g CO₂/km compared to ~180g/km of gasoline cars. Over a year, cycling just 10km daily offsets ~650 kg CO₂!";
      } else if (lower.includes("flight") || lower.includes("plane")) {
        botReply = "A single flight emits substantial emissions. A domestic round-trip average is ~0.8 to 1.2 Tons of CO₂ per passenger. Reducing flights in favor of trains or virtual syncs is highly carbon-effective.";
      }
    }
