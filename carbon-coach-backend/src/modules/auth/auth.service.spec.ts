...
describe("AuthService", () => {
  ...
  describe("register", () => {
    it("should register a user successfully and return tokens", async () => {
      const dto = { email: "new@example.com", password: "pwd", name: "New User" };
    ...
    it("should throw ConflictException if email is already registered", async () => {
      const dto = { email: "old@example.com", password: "pwd", name: "Old User" };
      jest.spyOn(prisma.user, "findUnique").mockResolvedValue({ id: "user-123" } as any);
  ...
  describe("googleLogin", () => {
    it("should create new google user if not exists", async () => {
      const profile = { email: "g@example.com", name: "G Name", googleId: "g123", picture: "g.png" };
      const mockCreated = { id: "u-g", email: "g@example.com", profile: { name: "G Name" } };
    ...
    it("should update user with googleId if existing user has no googleId", async () => {
      const profile = { email: "g@example.com", name: "G Name", googleId: "g123", picture: "g.png" };
      const mockExisting = { id: "u-g", email: "g@example.com", googleId: null, profile: { name: "G Name" } };

      jest.spyOn(prisma.user, "findUnique").mockResolvedValue(mockExisting as any);
  ...
  describe("refresh", () => {
    it("should refresh tokens successfully for existing user", async () => {
      jest.spyOn(prisma.user, "findUnique").mockResolvedValue({ id: "user-123", email: "u@example.com" } as any);
    ...
    it("should throw UnauthorizedException if user no longer exists", async () => {
      ...
      await expect(service.refresh("refresh-token-string")).rejects.toThrow(UnauthorizedException);
    });
  });
});
