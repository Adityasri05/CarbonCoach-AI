...
export class TwinService {
  ...
  async simulate(
    ...
    const simulatedEnergy = (energySlider * 12 * 7 * 0.4) / 1000 + (365 * 0.5 * 2) / 1000; // simulated ac default
    ...
    const simulatedScore = parseFloat(
      (simulatedTransport + simulatedEnergy + simulatedFood + simulatedFlights + wasteShoppingBaseline).toFixed(1)
    );
