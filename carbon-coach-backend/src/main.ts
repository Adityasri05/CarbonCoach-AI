...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription(
      'The complete API specifications document for CarbonCoach AI footprint awareness platform, tracking systems, vision cameras, and gamified community modules.',
    )
    .setVersion('1.0.0')
    .addBearerAuth()
    .build();
  ...
  console.log(
    `🚀 CarbonCoach AI backend compiling & running on http://localhost:${port}`,
  );
  console.log(
